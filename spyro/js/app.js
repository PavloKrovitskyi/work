(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new core(".feedback__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            autoHeight: false,
    ...
    da.init();
    const cards = document.querySelectorAll(".workout-item");
    cards.forEach((card => {
        card.addEventListener("click", (() => {
            cards.forEach((c => c.classList.remove("card-open")));
            document.querySelectorAll(".workout-item__text").forEach((t => t.classList.remove("text-open")));
            card.classList.add("card-open");
            const cardNumber = card.id.split("-")[1];
            const text = document.querySelector(`#text-card-${cardNumber}`);
            text.classList.add("text-open");
        }));
    }));
    const playVideoButton = document.querySelector(".video-page__play-button");
    const promoVideo = document.getElementById("promo-video");
    const videoTrialButton = document.querySelector(".video-page__button");
    promoVideo.volume = .7;
    playVideoButton.addEventListener("click", (() => {
        promoVideo.play();
        promoVideo.setAttribute("controls", "controls");
    }));
